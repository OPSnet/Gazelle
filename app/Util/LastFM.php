...
class LastFM extends \Gazelle\Base {
    ...
    public function artistEventList($ArtistID, $Artist, $Limit = 15) {
        $ArtistEvents = $this->cache->get_value("artist_events_$ArtistID");
        if ($ArtistEvents === false) {
            $ArtistEvents = $this->fetch("artist.getEvents", ["artist" => $Artist, "limit" => $Limit]);
            $this->cache->cache_value("artist_events_$ArtistID", $ArtistEvents, 432000);
        }
        return $ArtistEvents;
    }
    ...
    public function userInfo(\Gazelle\User $user): ?array {
        ...
        $Response = $this->cache->get_value("lastfm_user_info_$lastfmName");
        if ($Response === false) {
            ...
            $this->cache->cache_value("lastfm_user_info_$lastfmName", $Response, 86400);
        }
        return $Response;
    }
    ...
    public function compare(int $viewerId, $Username, $Limit = 15) {
        ...
        $Response = $this->cache->get_value("lastfm_compare_$viewername" . "_$Username");
        if ($Response === false) {
            $Response = json_encode($this->fetch("tasteometer.compare",
                ["type1" => "user", "type2" => "user", "value1" => $viewername, "value2" => $Username, "limit" => $Limit]));
            $this->cache->cache_value("lastfm_compare_$viewername" . "_$Username", $Response, 86400 * 7);
        }
        return $Response;
    }

    public function lastTrack($Username) {
        $Response = $this->cache->get_value("lastfm_last_played_track_$Username");
        if ($Response === false) {
            ...
            $this->cache->cache_value("lastfm_last_played_track_$Username", $Response, 7200);
        }
        return $Response;
    }

    public function topArtists($Username, $Limit = 15) {
        $Response = $this->cache->get_value("lastfm_top_artists_$Username");
        if ($Response === false) {
            sleep(1);
            $Response = json_encode($this->fetch("user.getTopArtists", ["user" => $Username, "limit" => $Limit]));
            $this->cache->cache_value("lastfm_top_artists_$Username", $Response, 86400);
        }
        return $Response;
    }

    public function topAlbums($Username, $Limit = 15) {
        $Response = $this->cache->get_value("lastfm_top_albums_$Username");
        if ($Response === false) {
            sleep(2);
            $Response = json_encode($this->fetch("user.getTopAlbums", ["user" => $Username, "limit" => $Limit]));
            $this->cache->cache_value("lastfm_top_albums_$Username", $Response, 86400);
        }
        return $Response;
    }

    public function topTracks($Username, $Limit = 15) {
        $Response = $this->cache->get_value("lastfm_top_tracks_$Username");
        if ($Response === false) {
            sleep(3);
            $Response = json_encode($this->fetch("user.getTopTracks", ["user" => $Username, "limit" => $Limit]));
            $this->cache->cache_value("lastfm_top_tracks_$Username", $Response, 86400);
        }
        return $Response;
    }

    public function weeklyArtists($Limit = 100) {
        $Response = $this->cache->get_value("lastfm_top_artists_$Limit");
        if ($Response === false) {
            $Response = json_encode($this->fetch("chart.getTopArtists", ["limit" => $Limit]));
            $this->cache->cache_value("lastfm_top_artists_$Limit", $Response, 86400);
        }
        return $Response;
    }

    public function hypedArtists($Limit = 100) {
        $Response = $this->cache->get_value("lastfm_hyped_artists_$Limit");
        if ($Response === false) {
            $Response = json_encode($this->fetch("chart.getHypedArtists", ["limit" => $Limit]));
            $this->cache->cache_value("lastfm_hyped_artists_$Limit", $Response, 86400);
        }
        return $Response;
    }

    public function clear($viewerId, $Username, $UserID) {
        $Response = $this->cache->get_value("lastfm_clear_cache_$UserID");
        if ($Response === false) {
            // Prevent clearing the cache on the same uid page for the next 10 minutes.
            $this->cache->cache_value("lastfm_clear_cache_$UserID", 1, 600);
            $this->cache->delete_value("lastfm_user_info_$Username");
            $this->cache->delete_value("lastfm_last_played_track_$Username");
            $this->cache->delete_value("lastfm_top_artists_$Username");
            $this->cache->delete_value("lastfm_top_albums_$Username");
            $this->cache->delete_value("lastfm_top_tracks_$Username");
            $viewername = $this->username($viewerId);
            if (!is_null($viewername)) {
                if (strcasecmp($viewername, $Username)) {
                    [$viewername, $Username] = [$Username, $viewername];
                }
                $this->cache->delete_value("lastfm_compare_{$Username}_$Username");
            }
        }
    }
    ...
    protected function fetch(string $Method, array $Args) {
        ...
        $RecentFails = (int)$this->cache->get_value($RecentFailsKey);
        if ($RecentFails > 5) {
        ...
        if (is_array($Args)) {
            ...
            if ($curl->fetch($Url)) {
                ...
                $this->cache->cache_value($RecentFailsKey, $RecentFails + 1, 1800);
                return false;
            }
        }
    }
