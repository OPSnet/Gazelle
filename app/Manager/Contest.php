...
class Contest extends \Gazelle\Base {
    ...
    public function create (array $info) {
        ...
        if (isset($info['pool'])) {
            self::$db->prepared_query("INSERT INTO bonus_pool (name, since_date, until_date) VALUES (?, ?, ?)",
    ...
    public function contestList(): array {
        self::$db->prepared_query("
         ...
         ");
         return array_map(fn($id) => new \Gazelle\Contest($id), self::$db->collect(0));
    }
    ...
    public function schedulePayout(): int {
        ...
        $contests = array_map(fn($id) => new \Gazelle\Contest($id), self::$db->collect(0));
        $totalParticipants = 0;
