...
header('Content-Type: ' . ($user->downloadAlt() ? 'text/plain' : 'application/x-bittorrent') . '; charset=utf-8');
header('Content-Disposition: attachment; filename="' . $torrent->torrentFilename($user->downloadAlt(), MAX_PATH_LEN) . '"');
