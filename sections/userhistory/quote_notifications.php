    ...
    switch ($Result['Page']) {
        case 'forums':
            $Links = 'Forums: <a href="forums.php?action=viewforum&amp;forumid=' . $Result['ForumID'] . '">' . display_str($Result['ForumName']) . '</a> &rsaquo; ' .
                    '<a href="forums.php?action=viewthread&amp;threadid=' . $Result['PageID'] . '" class="tooltip" title="' . display_str($Result['ForumTitle']) . '">' . shortenString($Result['ForumTitle'], 75) . '</a>';
        ...
        <tr class="colhead_dark notify_<?=$Result['Page']?>">
            <td colspan="2">
                <span style="float: left;">
                    <?=$Links?>
                    &rsaquo; Quoted by <?=Users::format_username($Result['QuoterID'], false, false, false, false) . ' ' . time_diff($Result['Date']) ?>
