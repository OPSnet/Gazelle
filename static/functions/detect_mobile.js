...
var MOBILE_SITE_HOSTNAME = "xanax.rip";
var MOBILE_SITE_URL = "https://xanax.rip/";
var ANDROID_APP_URL = "https://xanax.rip";
...
if (window.location.hostname == MOBILE_SITE_HOSTNAME) {
	...
	if (isMobile.Android()) {
		if (!hasCookie(ANDROID_COOKIE_NAME)) {
			...
			var result = confirm("An Android app is available for Xanax.rip. Would you like to download it?");
			if (result == true) {
		...
		if (!hasCookie(OTHER_COOKIE_NAME)) {
			setCookie(OTHER_COOKIE_NAME, true, 365);
			var result = confirm("A mobile version of Xanax.rip is available. Would you like to use it?");
			if (result == true) {
